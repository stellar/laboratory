...
test.describe("View XDR to JSON Page", () => {
  ...
  test("Import ConfigUpgradeSet XDR (Protocol 26)", async ({ page }) => {
    const xdrInput = page.getByLabel("Base64 encoded XDR");
    await xdrInput.fill(MOCK_CONFIG_UPGRADE_SET_XDR);
    ...
    // Select ConfigUpgradeSet type from options
    await xdrTypeInput.focus();
    await expect(xdrTypeOptions).toBeVisible();
    await page.getByText("ConfigUpgradeSet").first().click();

    // Decode XDR for the correct type
    await expect(xdrTypeInput).toHaveValue("ConfigUpgradeSet");
    await expect(decodeErrorMsg).toBeHidden();
    await expect(jsonView).toBeVisible();

    // Verify frozen_ledger_keys_delta content is rendered
    await expect(jsonView).toContainText("frozen_ledger_keys_delta");
    await expect(jsonView).toContainText("keys_to_freeze");
    await expect(jsonView).toContainText("keys_to_unfreeze");
    await expect(jsonView).toContainText(
      "1804c340befaed93362504c1106e0143958a3903460065990d43781984f650834f114dd14f6500315647",
    );
    await expect(jsonView).toContainText(
      "18240258440320201257747bdfb2cf58f0c9dc53c10ba5cf59728f2432f6d832d05463890141f90c1248",
    );
  });
...
const MOCK_CONFIG_UPGRADE_SET_XDR =
  "";
