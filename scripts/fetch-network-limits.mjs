...
/**
 * Extracts raw JSON config settings from the RPC response.
 * Returns the data in the { updated_entry: [...] } format.
 *
 * @param {object} response - The RPC getLedgerEntries response
 * @returns {{ updated_entry: Record<string, unknown>[] }} Raw JSON config settings
 *
 * @example
...
const getRawJsonFromResponse = (response) => {
  ...
  for (const entry of entries) {
    ...
    // Skip internal settings not part of ConfigUpgradeSet
    if (INTERNAL_CONFIG_SETTINGS.has(configSettingId)) continue;

    const value = configSetting[configSettingId];
    if (value !== undefined) {
      updatedEntry.push({ [configSettingId]: value });
    }
  }

  return { updated_entry: updatedEntry };
};
...
const generateTypeScriptFile = (networkLimits, networkLimitsJson) => {
...
`;
  ...
  // Network limits JSON
  for (const [networkName, json] of Object.entries(networkLimitsJson)) {
    content += `export const ${networkName.toUpperCase()}_LIMITS_JSON: NetworkLimitsJson = ${JSON.stringify(json, null, 2)};

`;
  }

  // All limits JSON
  content += `export const NETWORK_LIMITS_JSON: Record<string, NetworkLimitsJson> = {
`;
  for (const networkName of Object.keys(networkLimitsJson)) {
    content += `  ${networkName}: ${networkName.toUpperCase()}_LIMITS_JSON,
`;
  }
  content += `} as const;
`;
