...
interface SubmitStepContentProps {
  xdrBlob: string;
  /** When true, forces RPC as the default submit method when RPC is available. */
  isSoroban?: boolean;
  xdrPickerDisabled?: boolean;
  onReset: () => void;
  /** Called with the stringified submit response on success. */
  onSuccess?: (result: string) => void;
}

/**
 * Submit step content shared by the build and fee-bump flows.
 *
 * Displays the transaction XDR alongside its hash and decoded JSON, then
 * submits via RPC or Horizon. On success, calls `onSuccess` with the
 * stringified response. On clear, calls `onReset`.
 *
 * @param xdrBlob - Base-64 encoded transaction envelope XDR to submit
 * @param isSoroban - Forces RPC as the default method when RPC is available
 * @param xdrPickerDisabled - Disables the XDR picker input (fee-bump flow)
 * @param onReset - Callback to reset the parent flow
 * @param onSuccess - Called with stringified response once submitted successfully
 */
export const SubmitStepContent = ({
  xdrBlob,
  isSoroban = false,
  xdrPickerDisabled = false,
  onReset,
  onSuccess,
}: SubmitStepContentProps) => {
  ...
  useEffect(() => {
    if (isSubmitRpcSuccess && submitRpcResponse) {
      onSuccess?.(JSON.stringify(submitRpcResponse));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isSubmitRpcSuccess, submitRpcResponse]);
  ...
  useEffect(() => {
    if (isSubmitHorizonSuccess && submitHorizonResponse) {
      onSuccess?.(JSON.stringify(submitHorizonResponse));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isSubmitHorizonSuccess, submitHorizonResponse]);
